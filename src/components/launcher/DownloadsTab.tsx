      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            <div className="text-right mr-2">
              ...
              <p className="text-sm font-mono font-bold text-foreground">{isPaused || isExtracting || isResolving ? '--:--' : formatETA(download.downloadedSize, download.totalSize, download.speed)}</p>
            </div>
        ...
        <div className="relative">
          <div className="h-3 bg-muted/50 rounded-full overflow-hidden">
            <div className={cn("h-full rounded-full transition-all duration-300", isPaused ? "bg-gradient-to-r from-yellow-500 to-orange-500" : isExtracting ? "bg-gradient-to-r from-yellow-500 to-orange-500 animate-pulse" : isResolving ? "bg-gradient-to-r from-blue-500 to-cyan-500 animate-pulse" : "bg-gradient-to-r from-primary via-purple-500 to-primary")} style={{ width: `${progressPercent}%` }} />
          </div>
          <div className="flex justify-between items-center mt-2">
            <div className="flex items-center gap-2">
              {isPaused ? <Pause className="w-4 h-4 text-yellow-400" /> : isExtracting ? <Archive className="w-4 h-4 text-yellow-400 animate-pulse" /> : isResolving ? <Loader2 className="w-4 h-4 text-blue-400 animate-spin" /> : <Zap className="w-4 h-4 text-primary" />}
              <span className="text-xs text-muted-foreground">{isPaused ? 'إيقاف مؤقت - اضغط استئناف للمتابعة' : isExtracting ? 'جاري فك الضغط...' : isResolving ? 'جاري استخراج الرابط المباشر...' : 'جاري التنزيل...'}</span>
            </div>
